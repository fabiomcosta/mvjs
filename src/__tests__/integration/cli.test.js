  ...
  test('ignores paths matching multiple patterns', async () => {
    await createTmpFs(
      ...
      async ({cwd, exec}) => {
        ...
        expect(await readFileString(path.join(cwd, './modules.d.js'))).toEqual(
          `import './a.js'`
        );
      }
    );
  ...
  test('updates imports for paths with any extension on multiple source paths', async () => {
    await createTmpFs(
      ...
      async ({cwd, exec}) => {
        ...
        expect(await readFileString(path.join(cwd, './b.sass'))).toEqual(
          '@import "./c.sass";'
        );
      }
    );
    ...
    await createTmpFs(
      ...
      async ({cwd, exec}) => {
        ...
        expect(await readFileString(join('./modules.js'))).toEqual(
          `import './c/d/foo.js'; import './c/d/b/bar.js';`
        );
